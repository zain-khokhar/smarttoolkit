...
export default function ImageConverter() {
  ...
  const fileAccept = useMemo(() => {
    // Build a good accept string based on config mime + extension
    const parts = [];
  ...
  const reset = () => {
    setFiles([]); setSelected([]); setConverted([]);
    setProgress(0); setLoading(false); setError(null);
  };
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-black flex flex-col items-center py-10 px-4">
      <Card className="w-full max-w-4xl shadow-2xl rounded-2xl">
        <CardContent className="p-6 space-y-6">
          <h1 className="text-3xl font-bold text-center">Universal Image Converter</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <Select value={sourceFormat} onValueChange={setSourceFormat}>
                <SelectTrigger className="w-28"><SelectValue placeholder="Source" /></SelectTrigger>
                <SelectContent>
          ...
          >
            <input
              ...
              className="hidden"
            />
            <label htmlFor="fileInput" className="text-blue-600 font-semibold cursor-pointer">
            ...
            <Button variant="secondary" size="sm" onClick={handleUrlUpload} className="mt-3">
              Upload from URL
            </Button>
          </div>
          ...
          <div className="space-y-4">
            {FORMAT_CONFIG[targetFormat].supportsQuality && (
              <div>
                <p className="text-sm font-medium">Quality ({Math.round(quality * 100)}%)</p>
                <Slider min={0.2} max={1} step={0.1} value={[quality]} onValueChange={(v) => setQuality(v[0])} />
              </div>
            )}
                  ...
                  {files.map((f) => (
                    <div
                      ...
                      className={`border rounded-lg p-3 flex flex-col items-center min-w-[200px] ${
                        selected.includes(f.id) ? "ring-2 ring-blue-500" : "ring-0"
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <img src={URL.createObjectURL(f.file)} alt={f.file.name} className="h-32 object-contain mb-1" />
          ...
          {loading && (
            <div className="space-y-2 mt-4">
              ...
              <p className="text-center text-sm text-gray-500">Processing {progress}%</p>
            </div>
          )}
          ...
          {converted.length > 0 && (
            <div className="mt-6">
              ...
              <div className="overflow-x-auto pb-4">
                <div className="flex space-x-4 min-h-max">
                  {converted.map((f) => (
                    <div key={f.id} className="border rounded-lg p-3 bg-gray-50 dark:bg-gray-800 flex flex-col items-center min-w-[200px]">
                      <h3 className="text-sm font-medium mb-2 text-center">
                        Converted {sourceFormat.toUpperCase()} → {targetFormat.toUpperCase()}
                      </h3>
                      <img src={f.url} alt={f.name} className="h-32 object-contain mb-1" />
                      {/* bugfix: use sizeLabel */}
                      <p className="text-xs text-gray-500 mb-2">{f.sizeLabel}</p>
                      <Button
